...
//    }
    ...
    init {
        ...
        channel = if (!allowSelfSignedCertificate) {
            ManagedChannelBuilder.forAddress(address.host, address.grpcPort)
                .useTransportSecurity()
                .build()
        } else {
            NettyChannelBuilder.forAddress(address.host, address.grpcPort)
                .sslContext(
